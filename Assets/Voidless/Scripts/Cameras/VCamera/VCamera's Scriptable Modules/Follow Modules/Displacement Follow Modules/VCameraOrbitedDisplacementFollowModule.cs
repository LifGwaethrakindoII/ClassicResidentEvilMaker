...
namespace Voidless
{
[CreateAssetMenu(menuName = PATH_FOLLOWMODULE + "Orbit-Follow")]
public class VCameraOrbitedDisplacementFollowModule : VCameraDisplacementFollowModule
{
	[Space(5f)]
...
#region Getters/Setters:
	...
	protected override Vector3 GetOffsetPositionRelativeToTarget(VCamera _camera, Vector3 _target)
	{
		AdjustScalar();

		Vector3 scaledOffset = (offset.normalized /** _camera.distanceAdjuster.distance * distanceScalar.y*/);
		Vector3 point = _target + (orbitRotation.rotation * (relativeToTarget ? (/*_camera.targetRetriever.GetTargetRotation() * */scaledOffset) : scaledOffset));
